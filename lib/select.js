...
module.exports.extend = function(db)
{
    db.select = function(query, callback)
    {
        ...
        {
            if (query.start) {
                sql += " limit " + query.start + "," + query.limit;
            } else {
        ...
        db.client.query(sql, function(error, seldata, fields)
        {
            db.client.query("select FOUND_ROWS() cnt", function(error, cntdata) {
                db.returnResults(callback, {data: seldata, count: cntdata[0].cnt});
            });
        });
    };
    ...
    db.selectOne = function(query, callback)
    {
        ...
        {
            db.returnResults(callback, {data: (list && list.length > 0) ? list[0] : null });
        });
    };
